...
sequenceDiagram
    ...
    loop every 5 s, up to 25 due rows, oldest first
        ...
        PUB->>RS: emails.send from FRED fred@fredvested.com, Reply-To help@fredvested.com, html and text (no list headers)
        RS-->>PUB: message id
...
6. **Send.** `EmailService.send(to, subject, html, text, headers)` calls Resend `emails.send` with from, to, subject, html, text and, when the map is non-empty, `headers`, and returns Resend's message id. The publisher passes NO `List-Unsubscribe` headers on any email (Andrew, 2026-09-25): mail clients label any message carrying them as list mail (Apple Mail's "this message is from a mailing list" strip), and the footer unsubscribe link is the opt-out, which satisfies CAN-SPAM. The RFC 8058 pair (`List-Unsubscribe`, `List-Unsubscribe-Post: List-Unsubscribe=One-Click`) is what Gmail and Yahoo require from senders above roughly 5,000 messages a day; if the list ever reaches that volume, add it back in `EmailOutboxPublisher.publish` per template. `POST /api/waitlist/unsubscribe` already honours the one-click body (section 4), so nothing else changes then. `EmailOutboxPublisherTest` asserts the headers map is empty for both templates.
...
- Unsubscribe stays a click on the footer link, on every email (Andrew, 2026-09-25: no `List-Unsubscribe` headers on any message, because mail clients then label the message as list mail). The footer link satisfies CAN-SPAM. `POST /api/waitlist/unsubscribe` also honours an RFC 8058 one-click body, so the headers can be added back per template if the Gmail and Yahoo bulk-sender threshold (about 5,000 messages a day) is ever reached.
