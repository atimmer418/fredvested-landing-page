...
### A4. Playwright enforcement (planned, Phase 8)

Not in the working tree as of 2026-09-24: there is no `tests/` directory, no Playwright config and no Playwright dependency in `frontend/package.json`. The Phase 8 test is to live at `tests/analytics.spec.js`. What it must do, so A1 does not depend on a human:

- For each page above, subscribe to `page.on('request')`, collect `new URL(request.url()).hostname`, and fail the test if any hostname is outside the allow-list: the page's own host, `*.fredvested.com`, `challenges.cloudflare.com`.
- Fail explicitly on `plausible.io` (the proxy exists so that host is never contacted by a browser).
- Run against a preview with `localStorage.fred_analytics_force = '1'` set in the context so the analytics requests exist to inspect (B2), and assert the `/api/event` bodies per B5 and B7. The upstream Plausible script also drops every event when `navigator.webdriver` is true (which Playwright sets) unless `window.__plausible` is set, so the test must set `window.__plausible = true` in an init script or no `/api/event` request will ever appear.
...
- `tests/analytics.spec.js` does not exist yet and neither does a Playwright setup (no config, no dependency). Whether it lives at the repo root or under `frontend/`, and how it runs against a preview, are to be decided in Phase 8.
- Whether Turnstile loads anything from a host other than `challenges.cloudflare.com` has not been observed in this repo; the first A1 run should record what the widget requests.
- The once-per-visit guard is per page load, not per browser session (B5). If the "7 per visitor" budget was meant per session, that needs storage that `attribution.js` deliberately avoids; Andrew's call.
...
- Forced preview events post to the production Plausible site. Whether previews should get their own Plausible site (a different `data-domain` per host) is a product decision for Andrew; nothing in the code decides it today (`data-domain="fredvested.com"` is hard-coded on every page).
