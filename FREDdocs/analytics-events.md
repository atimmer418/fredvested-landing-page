...
In one line (decision, 2026-09-25): `Waitlist Submitted` in Plausible includes re-submits of unconfirmed addresses; the database view `v_waitlist_funnel` counts distinct addresses and is authoritative for the submit-to-confirm step. Plausible was never the source of truth for that rate.
...
All state (`fired`, `failedCount`, edit counters, calculator context) is in memory. "Session" means one page load: a reload starts over, `index.html` and `about.html` are separate page loads, and nothing is written to storage for guarding, so a returning visitor counts again. This is the designed behaviour (decision, 2026-09-25): Plausible's goal reports count unique visitors per goal, so a visitor who navigates away and comes back is collapsed to one there anyway, and guarding across page loads would need storage that `attribution.js` deliberately avoids. `track()` and `recordInput()` catch every exception; analytics cannot break the calculator or block a signup.
